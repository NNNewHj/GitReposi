...
![659828-20160603110004992-164557128](Java 容器.assets/659828-20160603110004992-164557128.png)

![full_container_taxonomy_thinking_in_java](Java 容器.assets/full_container_taxonomy_thinking_in_java.png)
...
添加元素时使用 ensureCapacityInternal() 方法来保证容量足够，如果不够时，需要使用 grow() 方法进行扩容，新容量的大小为 `oldCapacity + (oldCapacity >> 1)`，也就是旧容量的 1.5 倍，如果需要的容量大于旧容量的1.5倍，则直接使用需要的容量作为新的容量。
...
- HashMap 可以插入键为 null 的 Entry，而HashTable不可以。
- HashMap 的迭代器是 fail-fast 迭代器，当有其它线程改变了HashMap的结构（增加或者移除元素），将会抛出ConcurrentModificationException，但迭代器本身的remove()方法移除元素则不会抛出ConcurrentModificationException异常。
